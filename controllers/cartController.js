...
const clearCart = async(req,res) => {
	try{
		...
		const userExists = await knex.select('user_id').from('users').where('user_id',userId);
		if(!userExists || userExists.length == 0){
			return res.status(400).json({message: 'User not found'});
		}

		const userCart = await knex.select('cart_id').from('cart').where('user_id',userId);
		if(!userCart || userCart.length == 0){
			return res.status(400).json({message: 'User cart does not exists'});
		}

		const userCartDetails = await knex.select('cart_detail_id').from('cart_detail').where('cart_id',userCart[0].cart_id);
		if(!userCartDetails || userCartDetails.length == 0){
			return res.status(400).json({message: 'User cart is empty'});
		}

		await knex('cart_detail').where('cart_id',userCart[0].cart_id).del();
		res.status(200).json({message: 'User cart cleared successfully'});
		
	}catch(err){
		console.error(err);
		res.status(500).json({message: 'An error occured while clearing user cart'});
	}
}
