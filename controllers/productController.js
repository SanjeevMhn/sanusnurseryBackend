...
const countAllProducts = async (req, res) => {
  try {
    ...
    res.status(200).json({ count: prodCount.rows[0].count });
  } catch (err) {
    console.log(err);
    res
      .status(500)
      .json({
        message: "An error occured while getting total products number",
      });
  }
};

const countAllProductCategories = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occured while getting total categories" });
  }
};
...
const getAllProducts = async (req, res) => {
  try {
    ...
    const products = await knex.raw(`select 
                                      ...
                                      order by pd.prod_name asc limit ? offset ?`,
      [limit, offset],
    );
    ...
    res
      .status(500)
      .json({ message: "An error occured while getting product data" });
  }
};
...
const getProductsByCategory = async (req, res) => {
  try {
    ...
    const products = await knex.raw(`select 
                                            ...
                                            inner join product_categories pc on pd.prod_category = pc.prod_cat_id 
                                            inner join product_image_details pim on pd.prod_id = pim.product_id 
                                            where pc.prod_cat_name = ? limit ? offset ?`,
      [category, limit, offset],
    );
    const totalCount = await knex.raw(
      "select count(*) from products pd join product_categories pc on pd.prod_category = pc.prod_cat_id where pc.prod_cat_name = ?",
      [category],
    );
    res.status(200).json({
      currentPage: page,
    ...
    res
      .status(500)
      .json({ message: "An error occured while retrieving product data" });
  }
};
...
const getRelatedProducts = async (req, res) => {
  try {
    ...
    const products = await knex.raw(`select 
                                        ...
                                        inner join product_categories pc on pd.prod_category = pc.prod_cat_id
                                        inner join product_image_details pim on pd.prod_id = pim.product_id 
                                        where pc.prod_cat_name = ? and pd.prod_id <> ? limit ? offset ?`,
      [category, excludeId, limit, offset],
    );
    const totalCount = await knex.raw(
      "select count(*) from products pd join product_categories pc on pd.prod_category = pc.prod_cat_id where pc.prod_cat_name = ? and pd.prod_id <> ?",
      [category, excludeId],
    );
    ...
    res
      .status(500)
      .json({
        message: "An error occured while retriving related products data",
      });
  }
};
...
const getProductCategories = async (req, res) => {
  try {
    const productCategories = await knex
    ...
    res
      .status(500)
      .json({ message: "An error occured while getting product categories" });
  }
};
...
const addProduct = async (req, res) => {
  try {
    ...
    const prod_category_id = await knex.select('prod_cat_id').from('product_categories').where('prod_cat_name', prod_category);
    ...
    let insertObj = {
      prod_name: prod_name,
      prod_category: prod_category_id[0].prod_cat_id,
      prod_price: num_prod_price,
      prod_inStock: prod_inStock,
    };

    let lastInserId = await knex("products")
      .returning("prod_id")
      .insert(insertObj);
    ...
    res
      .status(500)
      .json({ message: "An error occured while adding new product data" });
  }
};
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      const productOldImg = await knex
        .select("*")
        .from("product_image_details")
        .where("product_id", prod_id);
      await cloudinary.uploader.destroy(
        productOldImg[0].public_id,
        productOldImg[0].signature,
      );
      ...
      await knex("products").where("prod_id", prod_id).update(updatedObj);
      ...
      await knex("products").where("prod_id", prod_id).update(updates);
    }
    ...
    res
      .status(500)
      .json({ message: "An error occured while updating product data" });
  }
};
...
const getProductCategoryById = async (req, res) => {
  try {
    ...
    const productCatName = await knex
      .select("prod_cat_id", "prod_cat_name")
      .from("product_categories")
    ...
    res
      .status(500)
      .json({ message: "An error occured while getting the product category" });
  }
};
...
const deleteProduct = async (req, res) => {
  try {
    ...
    const productImgDetails = await knex("product_image_details").where(
      "product_id",
      prod_id,
    );
    await cloudinary.uploader.destroy(
      productImgDetails[0].public_id,
      productImgDetails[0].signature,
    );
    ...
    res
      .status(500)
      .json({ message: "An error occured while deleting the item" });
  }
};

module.exports = {
  countAllProducts,
  countAllProductCategories,
  getAllProducts,
